...
// Копието за гледане на цял екран. 1440 пиксела са колкото най-широкия
// телефонен дисплей, тоест повече не би се видяло. Файлът пада до към 200 KB и,
// по-важното, разкодирането му заема под 8 MB памет вместо над 20.
export const VIEW = { maxWidth: 1440, quality: 0.82 };

// Снимката на човека. Показва се в кръгче от 30 до 96 пиксела, тоест 256 са
// запас дори при най-плътните екрани. Файлът излиза към 20 KB — колкото едно
// умалено копие в чата, а се тегли по веднъж на екран за всички наведнъж.
export const AVATAR = { maxWidth: 256, quality: 0.8 };
...
// Изрязване на квадрат от снимката. Правоъгълникът е в пиксели на оригинала.
// Връща оригиналния адрес при неуспех — тогава кръгчето показва средата, както
// преди да има изрязване.
export async function cropTo(uri, rect) {
  const M = load();
  if (!M || typeof M.manipulateAsync !== "function" || !rect) return uri;
  try {
    const result = await M.manipulateAsync(
      uri,
      [{ crop: rect }],
      { compress: 1, format: M.SaveFormat?.JPEG || "jpeg" }
    );
    return result?.uri || uri;
  } catch {
    return uri;
...
export async function makeThumb(uri) {
  return makeCopy(uri, THUMB);
}

// Копие за гледане. Връща null при неуспех — тогава цял екран показва голямата
// снимка, както преди.
export async function makeView(uri) {
  return makeCopy(uri, VIEW);
}

async function makeCopy(uri, preset) {
  const M = load();
  if (!M || typeof M.manipulateAsync !== "function") return null;
  const out = await shrinkPhoto(uri, null, preset, true);
