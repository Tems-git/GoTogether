...
### Ден 9 — 8 август 2026 — първи Android тест
След около месец пауза приложението беше пуснато за пръв път на Android устройство (Samsung), успоредно с iPhone — два телефона, два различни акаунта, едно пътуване.

- **Supabase проектът беше паузиран** (free tier, ~месец без активност) → всяка заявка гърмеше с `Network request failed`, после HTTP 521 докато се вдигаше. Рестартиран през MCP `restore_project`. Виж новата секция "Supabase free tier паузира проекта".
- **Safe area на Android** — навигационната лента застъпваше полето за писане в чата дотолкова, че тапването задействаше системните бутони. Инсталиран `react-native-safe-area-context`, `App.js` обвит в `SafeAreaProvider`, и всичките седем екрана минаха от хардкоднати `paddingTop: 60` към реални `useSafeAreaInsets()`. Виж новата секция "Safe area".
